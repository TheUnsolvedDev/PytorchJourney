...
class LabelSmoothingLoss(torch.nn.Module):
    ...
    def forward(self, predictions, targets):
        one_hot = torch.zeros_like(predictions).scatter(
            1, targets.unsqueeze(1), 1)
        smoothed_labels = one_hot * \
            (1 - self.epsilon) + (1 - one_hot) * \
            (self.epsilon / (self.num_classes - 1))
        log_probabilities = torch.log_softmax(predictions, dim=1)
        loss = torch.sum(-smoothed_labels * log_probabilities, dim=1).mean()
        return loss
...
def train(train_loader, test_loader):
    ...
    criterion = LabelSmoothingLoss(num_classes=10)
    model.to(device)
