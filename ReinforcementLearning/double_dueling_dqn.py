...
class QNetwork(torch.nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.fc_advantage = torch.nn.Linear(64, output_dim)
        self.to(device)
...
def fit_dueling_double_dqn(states, actions, rewards, next_states, dones, gamma, q_net, target_q_net, optimizer):
    max_a_q_sp_actions = q_net(next_states).detach().max(1)[1].unsqueeze(1)
    max_a_q_sp_values = target_q_net(next_states).gather(1, max_a_q_sp_actions)

    target_q_s = rewards + gamma * max_a_q_sp_values * (1 - dones)
        ...
        while not done:
            ...
            if len(buffer) >= 5*BATCH_SIZE:
                for i in range(10):
                    ...
                    fit_dueling_double_dqn(*batched_experience,
