...
class InceptionBlock(torch.nn.Module):
    def __init__(self, im_channels, num_1x1, num_3x3_red, num_3x3, num_5x5_red, num_5x5, num_pool_proj):
        ...
        self.tree_by_three_red = ConvBlock(
            im_channels, num_3x3_red, kernel_size=1)
        self.tree_by_three = ConvBlock(
            num_3x3_red, num_3x3, kernel_size=3, padding=1)
        self.five_by_five_red = ConvBlock(
            im_channels, num_5x5_red, kernel_size=1)
        self.five_by_five = ConvBlock(
            num_5x5_red, num_5x5, kernel_size=5, padding=2)
...
def train(train_loader, test_loader):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            output_primary, output_secondary, output_tertiary = model(data)

            loss_primary = criterion(output_primary, target)
            loss_secondary = criterion(output_secondary, target)
            loss_tertiary = criterion(output_tertiary, target)

            loss = loss_primary + 0.5*loss_secondary + 0.5*loss_tertiary
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for data, target in test_loader:
                data, target = data.to(device), target.to(device)
                output_primary, _, _ = model(data)
                _, predicted_primary = torch.max(output_primary.data, 1)
                total += target.size(0)
                correct += (predicted_primary == target).sum().item()
